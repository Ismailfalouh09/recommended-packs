...
- Protected admin media module completed.
- Cloudinary is isolated behind `MediaStorageProvider` and `CloudinaryStorageProvider`.
- Actual image signatures are validated with `file-type`; JPEG, PNG, and WEBP are allowed.
- `OWNER` and `ADMIN` can upload, replace, reorder, update metadata, and delete images.
- `OWNER`, `ADMIN`, and `STAFF` can read generic media assets.
- Generic media endpoints remain:
  - `POST /admin/media/upload`
  - `GET /admin/media`
  - `GET /admin/media/:id`
  - `PATCH /admin/media/:id`
  - `DELETE /admin/media/:id`
- Entity media endpoints added:
...
- Prisma media relationships added:
  - `ProductImage`
  - `PackImage`
  - `CategoryImage`
  - `ProductReferenceImage`
  - `MediaRole`
- Product and pack cover-image behavior is enforced in service transactions.
- Category and product-reference replacement keeps old images until the new upload and database write succeed.
- Deletion removes database relationships first and then attempts provider cleanup.
- Optimized URL variants are generated by `MediaUrlService`; transformed URLs are not stored in PostgreSQL.
- Product, pack, category, product-reference, and recommendation responses now include image data additively.
