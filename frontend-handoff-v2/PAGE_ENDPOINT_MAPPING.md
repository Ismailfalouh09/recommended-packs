...
| Customer Store - Product details | GET /products/{id}; GET /products/slug/{slug}; GET /products/{slug}/reviews | Public | READY | Slug and ID detail routes exist; reviews are approved-only. Each reference now returns `galleryImages` (ordered, per-shade) and `primaryImageUrl`, separate from `swatch` and product `mediaGallery`. |
...
| Admin Dashboard - Products | GET/POST/PATCH/DELETE /admin/products; product image endpoints | Admin | READY | `POST /admin/products` accepts multipart creation with optional `coverImage` and repeated `images` fields; edit-time product image endpoints remain available. |
| Admin Dashboard - Product references / stock | GET/POST /admin/products/{productId}/references; GET/PATCH/DELETE /admin/product-references/{id}; PATCH /admin/product-references/{id}/stock; PUT/DELETE /admin/product-references/{referenceId}/image | Admin | READY | `POST /admin/products/{productId}/references` accepts multipart creation with optional `swatch`; reference stock and edit-time swatch endpoints remain available. |
| Admin Dashboard - Reference galleries (per-shade) | GET/POST /admin/product-references/{referenceId}/gallery-images; PATCH /admin/product-references/{referenceId}/gallery-images/reorder; PATCH /admin/product-references/{referenceId}/gallery-images/{imageId}; PATCH /admin/product-references/{referenceId}/gallery-images/{imageId}/primary; DELETE /admin/product-references/{referenceId}/gallery-images/{imageId} | Admin | READY | Multi-image per-shade gallery. Reads OWNER/ADMIN/STAFF, writes OWNER/ADMIN. First upload becomes primary; deleting the primary auto-promotes the next by position. |
...
| Admin Dashboard - Media | POST /admin/media/upload; GET /admin/media; GET/PATCH/DELETE /admin/media/{id}; entity image endpoints (product, pack, category, reference swatch, reference gallery) | Admin | READY | Cloudinary-backed provider layer. |
