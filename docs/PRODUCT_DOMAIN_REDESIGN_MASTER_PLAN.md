...
**Implementation progress (2026-06-29):** Phase 6 backend MVP is implemented for `FIXED_REFERENCE` and `AUTO_BEST_REFERENCE`. Pack admin validation rejects `CUSTOMER_CHOICE`, invalid fixed product/reference ownership, inactive fixed references, and unavailable fixed references on active packs. Recommendation selection now combines product-level general suitability with reference-level shade suitability, filters automatic references by active/in-stock state plus tone/undertone compatibility, and stores/returns the selected ProductReference for fixed and automatic items. Required items with no compatible available reference continue to make the pack ineligible under the current recommendation convention.
